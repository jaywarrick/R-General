...
myServer <- shinyServer(function(input, output, session) {
     ...
     output$plot <- renderPlotly({
          ...
          if(is.data.table(shinyData))
          {
               ...
               colorV <- as.character(shinyData[,get('NUCLEARRADIUS')])
               plot_ly(mode='markers', x=shinyData[,get(input$X)], y=shinyData[,get(input$Y)], color=colorV, text=shinyData[,get(input$Label)]) %>%
          ...
          else
          {
               ...
               ylab <- list(title = input$Y)
               colorV <- as.character(shinyData[,get('NUCLEARRADIUS')])
               plot_ly(mode='markers', x=shinyData[,input$X], y=shinyData[,get(input$Y)], color=colorV, text=shinyData[,get(input$Label)]) %>%
                    layout(xaxis = xlab, yaxis = ylab)
          }
