						  ...
						  cross.fun=median, cross.cex=3, cross.pch=10, cross.lwd=2.5, cross.args=list(), cross.plot=F, ...)
{
	...
	if(is.null(plot.by))
	{
		legend.colors <- data[, list(grp=paste0(lapply(.BY, as.character), collapse='.'), by.index=.GRP, my.color=my.temp.color[1]), by=by]
	}
	else
	{
		legend.colors <- unique(data[, append(mget(plot.by), list(grp=paste0(lapply(.BY, as.character), collapse='.'), by.index=.GRP, my.color=my.temp.color[1])), by=by])
	}
