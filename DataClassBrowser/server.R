...
myServer <- shinyServer(function(input, output, session) {
     ...
     output$plot <- renderPlotly({
          ...
          if(is.data.table(shinyData))
          {
               ...
               plot_ly(mode='markers', x=shinyData[,get(input$X)], y=shinyData[,get(input$Y)], color=shinyData$Class, text=shinyData[,get(input$Label)]) %>%
          ...
          else
          {
               xlab <- list(title = input$X)
               ylab <- list(title = input$Y)
               plot_ly(mode='markers', x=shinyData[,input$X], y=shinyData[,get(input$Y)], color=shinyData$Class, text=shinyData[,get(input$Label)]) %>%
                    layout(xaxis = xlab, yaxis = ylab)
          }
